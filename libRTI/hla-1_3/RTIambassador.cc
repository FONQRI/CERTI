...
std::vector<certi::RegionHandle> build_region_handles(RTI::Region** regions, int nb)
{
...
RTI::Handle get_handle(const RTI::Region& region)
{
    try {
...
char* hla_strdup(const std::string& s)
{
    try {
        size_t len = s.length();
...
#ifdef CERTI_REALTIME_EXTENSIONS
// ----------------------------------------------------------------------------
void RTI::RTIambassador::setPriorityforRTIAProcess(int priority, unsigned int sched_type)
{
#ifdef _WIN32
...
// ----------------------------------------------------------------------------
void RTI::RTIambassador::setAffinityforRTIAProcess(cpu_set_t mask)
{
#ifdef _WIN32
    throw RTIinternalError("Not Implemented on Windows");
#else
